...
if (!is_user_logged_in()) {
    echo "<p>Please login to view your classifieds.</p>";
    return;
}
...
if (!current_user_can('administrator')) {
    $args['author'] = $user_id; // normal users see only their ads
}
...
<div class='filters-row' style='display:flex;gap:10px;align-items:center;'>
        ...
        <div class="multi-select-dropdown">
            ...
            <div class="dropdown-content">
                <?php
                $terms = get_terms(['taxonomy'=>'freeads-category','hide_empty'=>false]);
        ...
        <a href="<?php echo $siteurl; ?>/dashboard/classified-ads/" class="btn reset">Reset</a>
    </div>
</form>
    ...
    if (isset($_GET['msg'])) {
		...
		if (current_user_can('administrator')) {
			 echo '<div style="padding:10px;background:#d4edda;color:#155724;margin-bottom:20px;">';
			echo ($_GET['msg']=='added') ? 'Classified Ad Added Successfully.' : 'Classified Ad Updated Successfully';
			echo '</div>';
		}else{
			 echo '<div style="padding:10px;background:#d4edda;color:#155724;margin-bottom:20px;">';
			echo ($_GET['msg']=='added') ? 'Classified Ad Added Successfully (Pending Review)' : 'Classified Ad Updated Successfully';
			echo '</div>';
		}
    ...
    <table style="width:100%;border-collapse:collapse;margin-top:20px;">
        ...
        <?php if ($ads->have_posts()) : while ($ads->have_posts()) : $ads->the_post(); 
            ...
            $cats = wp_get_post_terms($post_id, 'freeads-category', ['fields'=>'names']);
            $phone = get_post_meta($post_id, 'ad_tel', true);
            $email = get_post_meta($post_id, 'ad_email', true);
            $expiry = get_post_meta($post_id, 'ad_expiry', true);
        ...
        ?>
            ...
            <tr>
                ...
                <td style="padding:10px;border:1px solid #ddd;"><?php echo esc_html($phone); ?></td>
                <td style="padding:10px;border:1px solid #ddd;"><?php echo esc_html($email); ?></td>
                <td style="padding:10px;border:1px solid #ddd;"><?php echo !empty($expiry) ? $expiry : '<em>N/A</em>'; ?></td>
