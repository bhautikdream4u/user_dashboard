...
// Always sanitize POST if used
$start_date = isset($_POST['start_date']) ? sanitize_text_field($_POST['start_date']) : '';
$end_date   = isset($_POST['end_date']) ? sanitize_text_field($_POST['end_date']) : '';
$event_type = isset($_POST['event_type']) ? sanitize_text_field($_POST['event_type']) : '';
$location   = isset($_POST['location']) ? sanitize_text_field($_POST['location']) : '';
$booking_link = isset($_POST['booking_link']) ? esc_url($_POST['booking_link']) : '';
?>
...
<div class="email-box">
    ...
    <h2>
        <?php echo $is_admin ? 'New Event Submitted' : 'Your Event Was Submitted Successfully'; ?>
    </h2>

    <?php if (!$is_admin): ?>
        <p>Thank you for submitting your event. Our team will review it shortly and publish it once approved.</p>
    <?php endif; ?>
    ...
    <table>
            ...
            <td>
                <?php if (!empty($booking_link)) : ?>
                    <a href="<?php echo esc_url($booking_link); ?>" target="_blank">
                        <?php echo esc_html($booking_link); ?>
                    </a>
                <?php else : ?>
                    —
                <?php endif; ?>
		...
		<?php endif; ?>
        <tr>
            <td class="label">Featured Event</td>
